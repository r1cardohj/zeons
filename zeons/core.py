...
class Zeons:
    ...
    def route(self, url_path, methods=None):
        ...
        def decorated(view_func):
            self.url_map[url_path] = (
                [method.upper() for method in (methods or ["GET"])],
                view_func,
            )
            return view_func
...
class Response:
    ...
    def __init__(self, body=None, status_code=200, headers=None, err_why=None) -> None:
        ...
        if body:
            if isinstance(body, (dict, list)):
                ...
                self.headers["Content-Type"] = (
                    self.ContentTypes.HTML.value + "; charset=UTF-8"
                )
            elif isinstance(body, bytes):
                self.body = body
            else:
                raise ResponseTypeException(
                    "response type must be dict,list,str or bytes"
                )
